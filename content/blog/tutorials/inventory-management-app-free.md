...
Even better, since we’re going to use Clairdash to build it, you’ll barely need any coding skills - and we’ll have a fully functional application before your coffee gets cold.

All we need to do is connect our data, add a couple of custom queries, and start using Clairdash’s built-in components to create a professional UI for managing stock levels.
...
Along with Clairdash’s native *read* functionality, we’ll also need three custom queries.
...
If you haven’t already, you’ll need to sign up for a Clairdash account. Our pricing structure is the most generous in the low-code space, and you can do everything you need to build and deploy a fully-functioning inventory management tool with our free tier alongside paid plans with annual pricing or monthly fees.

### 1. Create a new app

We’ll start by hitting *create new app* on the Clairdash portal. Then press *start from scratch*. You’ll be prompted to choose a name and URL for your new app project:
...
Then, we’ll be asked to select our data source. Clairdash offers a market-leading range of external data connectors, as well as our own built-in database.
...
Once we’ve done this, we can use Clairdash’s *fetch tables* functionality to access our tables from the *data* tab of the builder.
...
- **Query name** - what we’ll call our query within Clairdash.
...
- **Transformer** - any JavaScript we want to apply to the data returned by our query before we pass it to Clairdash’s front-end or automation builder.
...
While we’re here, we need to add another action to set a *state*. Essentially, we’re going to place an *update* form in our side panel, so we need to tell Clairdash to load the data for the row that we’ve clicked on.
...
Now when we click on a card, Clairdash saves the relevant ProductID and then opens our side panel.
...
You can also check out our guide to building a [SQL GUI](https://clairdash.com/blog/tutorials/sql-gui/).
...
Clairdash leads the pack for external data support. We offer dedicated connectors for MySQL, SQL Server, Postgres, Airtable, S3, Mongo, Arango, Oracle, Couch, Redis, REST API, Google Sheets, and more.

We’ve even got our own built-in database to help get you up and running in seconds.

### Optional self-hosting

Security-conscious organizations love Clairdash for the power to deploy their tools however they choose. We offer optional self-hosting through Kubernetes, Docker, Digital Ocean, Portainer, and more.

Or, use Clairdash Cloud and let us handle everything. Check out our [pricing page](https://clairdash.com/pricing) to learn more.

### Automations & integrations

Clairdash makes it a breeze to automate all kinds of workflows and processes. Use our flow-based editor to combine built-in triggers and actions to create your perfect automations, with minimal coding skills.
...
Clairdash is the clear leader for extensibility in the low-code space. Build your own components, data sources, and automation blocks with our dedicated CLI tools - or import community contributions from GitHub at the click of a button.

Check out our custom [plug-ins documentation](https://docs.clairdash.com/docs/custom-plugin) to learn more.

### 50+ free application templates

Our users choose Clairdash to build all kinds of internal tools, web apps, and utilities, in minutes, not months. We’ve even built more than fifty free, customizable [app templates](https://clairdash.com/templates) to help get you started.
