...
Finally, we’ll look at the key use cases and business challenges you can solve using different data standards, and how Clairdash is leading the charge in simplifying the app-building process.
...
This has changed somewhat with the rise of low-code development and other tools. For example, Clairdash offers a built-in database, with support for CSV uploads.
...
More modern development tools have made this easier. For example, in no/low-code tools, like Clairdash, most of the work of creating a data access layer is done for you.
...
Others may provide subcategories of any type of data. For example, Clairdash’s built-in database offers a distinction between normal strings and long-form text variables.
...
For example, with Clairdash you can quickly add new columns within our built-in database, making it easy to add new attributes to any table, at the click of a button.
...
Remember that to create or update rows, we need to furnish our query with the values we want to store. [Forms](https://clairdash.com/forms) are the perfect way to do this using user-provided data. In the most basic terms, this means creating a query and tying each value to a form field.
...
Within Clairdash, to retrieve the price of a customer’s first order in JavaScript and bind it to a new column in the _customers_ table, we’d return something like _$(“orders.0.price”),_ as a formula variable for each _customers_ row.
...
#### Data bindings in Clairdash

For maximum power and ease of use, Clairdash offers data bindings with JavaScript and Handlebars expressions. To make life faster and easier, our bindings are fully contextual, with each possible option for any component or column displayed as a simple dropdown menu.
...
ClairdashDB, our built-in database, is an example of the latter.
...
Clairdash offers a public API, for exactly this reason.
...
Importing spreadsheet data means transferring it to a more sophisticated database standard. For example, by building SQL tables. Alternatively, app builders with a built-in database, like ClairdashDB, allow you to simply import CSV files.
...
For example, Clairdash supports Google Sheets as an app data source. Just authenticate your Google account, provide your spreadsheet ID or URL, and select the tables you need to query.
...
For example, Clairdash offers data connectivity for Airtable, MongoDB, CouchDB, Oracle, and more.
...
For example, in Clairdash, you can define these rules when you’re setting up your external data connection, using our built-in JavaScript editor.
...
Clairdash offers built-in connections for a range of external data sources, including:
...
Each of these has its own unique connection process. You can [check out our docs ](https://docs.clairdash.com/)to find out more.
...
The rise of low-code tools, like Clairdash, is changing the way businesses handle data. More and more organizations are choosing low-code platforms for faster, cheaper builds, and improved developer experiences.
...
The best low-code platforms offer better, faster experiences for developers of all levels. For instance, Clairdash offers:
...
![Add a member screenshot](https://res.cloudinary.com/daog6scxm/image/upload/v1653314881/cms/Clairdash_add_a_member_jnlqe3.png "Add a member screenshot")
...
## Configuring data sources with Clairdash

Clairdash is the fast, easy way to build tools around existing data sources. Alternatively, our low-code platform is perfect for building applications to gather business data. Countless businesses around the world trust Clairdash to build secure, effective tools to manage data.

We’re on a mission to change the way businesses build internal tools. Here’s how you can use Clairdash to build data-centric applications.

### Connect to external data

Clairdash makes it easy to query and leverage all kinds of external data. We offer an extensive range of built-in data connectors.
...
Check out [our documentation](https://docs.clairdash.com/) to find out more about connecting to external data from different sources.

### Use our built-in database

We’re biased, but we think ClairdashDB is the fastest way to create a new data source. Our built-in database makes it easy to build a schema from scratch, or import existing flat files. You can create a fully functioning app database, in minutes.

We’ve provided a streamlined UI for creating columns, adding rows, and setting rules.

ClairdashDB offers an extensive range of data types, with simple, user-friendly labels. We even offer advanced data types, including file attachments, long-form text fields, JSON objects, S3 attachments, and more.
...
![Manage data sources in Clairdash screenshot](https://res.cloudinary.com/daog6scxm/image/upload/v1653314661/cms/Manage_data_with_Clairdash_Screenshot_nqoz14.png "Manage data sources with Clairdash Screenshot")

## Manage data with Clairdash

Quickly create seamless apps for users to manage and access data. With a range of bindable components, Clairdash is the easy way to query, manipulate and view all kinds of data.

#### Data provider components

One of the key building blocks of any Clairdash app is the _Data Provider_ component. This can be configured to any data source you have set up in your app’s back end. Any component which is nested within this can then access the data source, through contextual bindings.
...
One of the key benefits of using Clairdash is cutting development time to an absolute minimum. Our autogenerated CRUD screens are a key part of this.
...
Our incredible forms are the ideal way to build effective data entry UIs. Clairdash offers more than 15 different form components, for different data types and user experience goals. Autogenerate simple forms, or build complex, multi-step interfaces from scratch in seconds.
...
Data tables are at the heart of many Clairdash apps. Display values to users in a readable, customizable table. With Clairdash, you have complete control over which values to present, as well as how they’re formatted.
...
Clairdash offers a range of powerful data visualization tools, including a wide variety of charts and graphs. Bind different attributes to our charts, or use views within your data source to create and visualize aggregate values.
...
Once you’ve configured a data source in Clairdash, setting up automated queries is a breeze. We offer a simple step-based UI for creating complex automated processes. Use WebHooks or Zapier to send data to and from your Clairdash app.
...
Or, automations can be used to notify different stakeholders when particular changes are made to your data from within Clairdash.

### Creating a data layer in Clairdash

Our users choose Clairdash to build applications for handling all kinds of data. From simple internal tools to complex integrated workflows, our low-code platform works perfectly with all manner of data sources.
...
## Connect your data and start using Clairdash today
...
At Clairdash, we’re changing the way businesses build apps around data. You don’t need to be an expert in database management to build effective tools. With ClairdashDB and a range of external data connectors, anyone can create professional web apps, with minimal coding skills.
