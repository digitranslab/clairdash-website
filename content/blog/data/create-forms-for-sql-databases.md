...
description = "Learn how to create forms for SQL databases in less than 5 minutes with Clairdash"
...
In this article, we’ll dive into how you can do this using Clairdash, and create your SQL forms for free in just a few minutes.
...
The first thing we need to do is create a new Clairdash app. If you haven’t already, sign up for a free account using the link below.
...
Once we’ve created our app, the first thing we need to do is choose a data source. Clairdash offers its own internal database, as well as a range of relational and non-relational DBMSs - along with Google Sheets, Airtable, REST APIs, and more.
...
We then have the option of choosing which of the tables within our database we want to *fetch* - pulling them into Clairdash.
...
Our database only has two tables, and we want both of them. Straight away, we can edit our data and the schema of each table within the backend of Clairdash:
...
We’ll start by building our two CRUD screens. In Clairdash, we can autogenerate these for connected SQL databases or internal tables. Head over to the *design* tab and we’ll be prompted with three options.
...
But, we want to tell Clairdash that the *content* attribute is a *long-form text* field. This will allow us to use slightly different front-end elements when we generate our new form.
...
Clairdash stands apart from traditional form builders for the power to add custom logic. For instance, we can easily add our own validation rules to any form field.
...
## Build secure, advanced forms with Clairdash

Clairdash is the open-source, low-code platform that turns data into action. IT teams across all industries choose our platform to build secure, advanced forms on top of all sorts of existing data sources.
...
Take a look at our [forms page](https://clairdash.com/forms/) to learn more.

Sign up to Clairdash today to start building performant forms in a fraction of the time.