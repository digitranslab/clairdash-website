...
At Clairdash, we’re on a mission to revolutionize the way businesses build tools, manage data, and streamline their processes.
...
## How to turn Excel spreadsheets into apps with Clairdash

With Clairdash, building custom business apps and web applications couldn’t be easier. There are actually two different ways that you can go about turning your spreadsheets into applications with our low-code development platform.
...
Let’s take a look at your options for beginning to move from Microsoft Excel to applications in Clairdash.

### Option 1 - Importing CSVs

First up, you can use ClairdashDB to turn your spreadsheets into a fully-fledged database. Our built-in database tool offers full support for CSV uploads. So, you can import individual Excel sheets and transform them into database tables, more or less instantaneously.
...
Once you’ve built a new app, you can simply select ClairdashDB as the data source, and upload your first file. Then, you can add more tables and upload more files, if you need to.
...
Clairdash offers a dedicated data connector for Google Sheets. Nowadays, there’s a fair chance you’re using Google’s Excel alternative anyway.

All you need to do is sign in to your Google account, and paste your spreadsheet’s URL into our connector UI. Clairdash will then pull in your existing Google Sheets data, in real-time. Once you publish your app, any update made in one platform will be reflected in the other.
...
The beauty of Clairdash is that you’re in control of how complex or simple you want to make your custom tools. Either way, you’ll enjoy the same intuitive, fast, and flexible developer experience.
...
### 1. Sign up to Clairdash and create a new app project

First of all, you’ll need to get set up with a Clairdash account and start a new app project. Clairdash is free to use, in the cloud, or self-hosted.

When you create your account, you’ll be prompted to choose between self-hosting and Clairdash Cloud. This is an important decision, as it will impact how you deploy your tools later.
...
Our [open-source, low-code platform](https://clairdash.com/blog/open-source-low-code-platforms/) is the ideal solution for enterprises, SMBs, and hobbyists alike.

Our mission is to build the world’s largest suite of open-source business solutions. We put our users on the front line of this mission. Use Clairdash to create the perfect custom tools for your needs, in minutes.

Check out our [pricing page](https://clairdash.com/pricing) for more information.
...
So, option number one is using ClairdashDB and uploading CSV files.

As you might have guessed, that means we need to select ClairdashDB as our [data source:](https://clairdash.com/blog/data/data-sources/)
...
Option two means connecting directly to a Google Sheets file as a data layer for our app. We’ll start by adding a new data source, just like before. Except this time, we’ll choose Google Sheets instead of ClairdashDB.

Then, we can sign in to our Google account, paste our spreadsheet URL into Clairdash data connector UI, and import each individual table that we want.
...
Once our data is successfully pulled into Clairdash, we can follow a few extra steps to make it our own. One of the really cool things about Clairdash is how our data tools bridge the gap between spreadsheets and databases, in terms of functionality, usability, and flexibility.
...
Clairdash is here to give you a headstart in any case.

We offer fully functioning, autogenerated [CRUD apps](https://clairdash.com/blog/crud-app/), at the press of a button. Use these screens for all kinds of data entry, management, and self-service applications.

Check out our ultimate guide to [form UI design](https://clairdash.com/blog/app-building/form-ui-design/) for inspiration.
...
Alternatively, your app might be more tightly focused on analyzing and presenting data. Clairdash makes this a breeze too.
...
Codifying your processes with Clairdash is the perfect way to introduce [workflow automation](https://clairdash.com/blog/automation/workflow-automation/) or simply build dedicated interfaces to make manual tasks faster and easier. We’ll return to automation in a few minutes.
...
Take a look at our guide to [integrating multiple databases](https://clairdash.com/blog/data/how-to-integrate-multiple-databases/).
...
Lucky for you, Clairdash makes it easy to build custom automations, without writing a single line of code.

Let’s take a quick example of how to build an automation in Clairdash. First thing’s first, head over to the _Automate_ tab at the top of your screen and choose _Create automation_:
...
Clairdash offers flexible [role-based access control](https://clairdash.com/blog/app-building/role-based-access-control/), with permissions assignable at the level of data, interfaces, or even individual components. Let’s take a look at how each of these works in turn.
...
Check out our in-depth guide on how to [implement RBAC](https://clairdash.com/blog/app-building/how-to-implement-rbac/) for more information.

### 8. Deploy your new app

Finally, it’s time to roll your new app out to real-world users. By now, you won’t be surprised to learn that Clairdash offers a few different options here. One is using Clairdash Cloud. As you can guess from the name, this is our proprietary cloud hosting tool.
...
Of course, the whole point of Clairdash is empowering you to create the perfect tools for your own needs. And maybe cloud deployment isn’t your thing.

We also offer optional self-hosting.

Deploy to your own infrastructure with Kubernetes, Docker, Docker Compose, and Digital Ocean. This could be a private cloud, local network, or even a single device. With Clairdash, you’re in control.
...
We’re taking the hard work out of building incredible tools. With Clairdash, you can build deployable solutions for all kinds of business problems, in minutes rather than months.

We’ve even built over 50 free, deployable [app templates](https://clairdash.com/templates/) to show off what Clairdash can do.
