...
Then, we’ll move on to how you can implement a successful RBAC system, including concrete steps you can follow, best practices, and how Clairdash makes it a breeze to assign and control user access.
...
You might also enjoy our in-depth resource on [ITIL change management](https://clairdash.com/resources/itil-processes/change-management/).
...
So, if you had an online [complaints record](https://clairdash.com/business-apps/templates/complaints-record-template/) tool, you might need three distinct roles, so that:
...
Take a look at our dedicated guide to the [differences between rule-based and role-based access control](https://clairdash.com/blog/app-building/difference-between-rule-based-role-based-access-control/) to learn more.
...
One classic example would be a simple internal tool, like an [employee directory](https://clairdash.com/business-apps/templates/employee-directory-template/).
...
We've also created a dedicated guide on [how to implement RBAC](https://clairdash.com/blog/app-building/how-to-implement-rbac/).
...
Note that what we call these doesn’t matter, so for now we'll stick with descriptive names. We’ll look at Clairdash’s naming system for roles a little later.
...
We’ll look at how to implement role-based access control in Clairdash a little later.
...
Check out our guide to [data access control](https://clairdash.com/blog/app-building/data-access-control/) to find out more.
...
Or, we could use conditionality rules on specific UI elements, to enforce our RBAC rules on a single screen. So, our [invoice submission tool’s](https://clairdash.com/approval-apps/templates/invoice-approval-software/) home screen might be based around a table for existing entries.
...
We’ll explore how Clairdash makes it easy to use any of these techniques a little later.
...
## Using role-based access control in Clairdash

Clairdash offers flexible, intuitive RBAC. We’ve made it easy to design, implement, and maintain role-based access control systems, with simple dropdown interfaces and pre-defined roles.
...
Clairdash offers four predefined roles, which you can assign to users from a simple dropdown menu. As we’ll see in a minute, you can then grant permissions relating to different screens, components, and queries for each of these.

Note that our platform has separate sets of roles and permissions for your apps’ end-users and your actual Clairdash installation.

Today, we’re talking specifically about end-user roles. If you want to know more about permissions within the Clairdash builder, you can find out more in our docs.
...
This means that Clairdash is perfect for creating public-facing web apps.
...
Permissions in Clairdash are primarily managed at the level of app screens. Whether you autogenerate screens or build them from scratch, you’ll need to designate the minimum role that users need to access each one.
...
With Clairdash, you can create a conditionality rule so that the table is filtered for entries created by the current user if they have a particular role. The same table will then display all entries to users with higher permissions.

### Custom queries

Clairdash also allows you to set permissions at the level of queries. This is particularly useful when working with external database schemas and sources, but we can also assign permissions for internal tables in ClairdashDB.
...
Tweak and perfect your RBAC system with role-based previewing in Clairdash. Test how workflows operate in practice, optimize each UI, and build the smoothest, most efficient tools possible.
...
Clairdash is built around an intuitive role-based access control system. With predefined roles, conditional UIs, and low-code interfaces for setting screen or query-level permissions, it’s easy to build an effective, adaptable RBAC system.

Sign up for free to start building secure, usable web apps with Clairdash today.