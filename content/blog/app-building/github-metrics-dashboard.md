...
Today we’ll learn how to build a custom tool with Clairdash.
...
Why not also check out our tutorial on [how to build a dashboard](https://clairdash.com/blog/tutorials/how-to-build-a-dashboard/)?
...
This component allows you to load metrics and KPIs for a quick overview. They can come from a direct API call or from other [data sources](https://clairdash.com/blog/data/data-sources/), including internal and external database tables. With them, you can highlight the main metrics of your project, and track progress on your default branch.

### Charts

![Bug Chart](https://res.cloudinary.com/daog6scxm/image/upload/v1655210222/cms/6_jjgeag.webp "Bugs Chart")

Charts allow team members to understand what is going on in more detail. In our GitHub dashboard, you'll see a way to create charts [combining multiple data sources](https://clairdash.com/blog/data/how-to-integrate-multiple-databases/). This is particularly useful when you can’t change the API response, and you need to merge data from different queries.
...
With the main components in mind, you can use Clairdash to build your prototype. Thus, you’ll get a feel of how your dashboard will look. We can edit this later as we begin to introduce different data sources.

If you haven’t already, sign up for a Clairdash account. It’s free, and you can our cloud hosting platform for quick deployment.
...
![Component select](https://res.cloudinary.com/daog6scxm/image/upload/v1655210363/cms/7_mdp5rn.webp "Clairdash builder select components")
...
These tokens work like a user/password access for your account. Thus, instead of loading the API path and nothing else, Clairdash is going to load the path and send the user credentials. This greatly increases the number of API requests you can make.
...
Thus, we preprocess it and save it in a ClairdashDB table.

We also need to save the repository as a setting. We use ClairdashDB for that too.
...
* **Value** - the setting value, such as Clairdash/clairdash.
...
This is what you’d use to create a binding called “repo” with the default value of “Clairdash/clairdash”. Then, you use this variable in the URL or in the params tab, depending on how your API call is created. Like so:
...
Take a look at our guide to [WebHooks vs APIs](https://clairdash.com/blog/inside-it/webhook-vs-api/).

The other special case is the bug chart. There isn’t a single API call to return data from multiple calls. Thus, you could use GraphQL, an external service to aggregate multiple API calls, or you can use Clairdash automations to preprocess your data.

If you want to use Clairdash, you can copy what we did for the settings table. Then, you just need to create an automation that updates this data from time to time. For example, if you want to update it daily, you can create an Automation, run it with a cron trigger, and here are the steps:
...
Let’s head back to our admin page. For demo purposes, it is a public page. But you can change which users can read it, with Clairdash’s simple [role-based access control ](https://clairdash.com/blog/app-building/role-based-access-control/)system.
...
The current values are stored in our ClairdashDB. This means that we need to make this information available to our page. You can load data from any of your data sources using the data provider component.
...
These helpers allow you to modify the data from a variable. They aren’t as flexible as pure JS code, but they are quite useful for simple operations. Check out the [full list of handlebars helpers in Clairdash](https://docs.clairdash.com/docs/bindings).
...
## Building a GitHub metrics dashboard in Clairdash

Today you’ve learned how to build a GitHub metrics dashboard. In addition, you’ve learned how development teams can load and process data using Clairdash. By the end of the day, you should be able to create your own dashboards, loading custom elements with filtering options, styling, charts, and more.

{{< cta >}}

To find out more about the kinds of tools you can create with Clairdash, have a look at our [product page](https://clairdash.com/product).

We hope you’ve enjoyed it, and see you again next time!

For more inspiration, check out our range of pre-built [app templates.](https://clairdash.com/templates)