...
date = "2023-03-20"
description = "We wanted to investigate whether we can get an RPA to ship Clairdash apps - and, more importantly, should you?"
...
We used a tool called UI.Vision to set up a bot that creates CRUD apps for Postgres in Clairdash.
...
- Specifically, we opted to try and get UI.Vision to output Postgres CRUD apps in Clairdash, with the minimum number of user prompts.
...
- However, to do so *securely* we’d have to limit ourselves to tables that are contained in data sources that are pre-configured in Clairdash as environment variables.
...
- **Is open-source** - Clairdash is too, and this is important to us.
...
So, creating a screen in an existing Clairdash app would look like this:
...
Therefore, we’re not storing either our Clairdash logins or the database credentials we need to build a CRUD app in UI.Vision. The former means we’d have to manually sign in to Clairdash as normal before we could initiate our macro.

Dealing with database credentials constrains us more in terms of how usefully we can combine RPA and low-code. 

Instead of passing our credentials to UI.Vision, we set up our macro to work with a pre-configured data source in Clairdash, using *environment variables* - which encrypt and store your credentials within your Clairdash instance.

That way, UI.Vision doesn’t know anything about our data other than the name of the table we’re querying.

The flip side is that we can only run our macro when the data source we need is already configured and usable within Clairdash. 
...
We should state for absolute clarity that environment credentials are a *paid* feature in Clairdash. 
...
One way of framing this is that part of the reason that it was so easy for us to create the macro that we did is that we picked a type of tool that can be built quickly in Clairdash in the first place.

It only took 43.77 seconds because Clairdash can already autogenerate CRUD screens, so our macro doesn’t exactly need to do much heavy lifting. This would probably only take a human user a couple of minutes, so the effective time-saving is actually pretty marginal.
...
It’s also worth noting that UI.Vision works by analyzing and then interacting with onscreen elements. Therefore, if you use it with Clairdash - or any other tool - and an update comes out that materially changes the UI, your macro is probably going to break.
...
You might also like our round-up of [no-code tools](https://clairdash.com/blog/no-code-tools/).

We kicked off with two questions. Let’s answer each one in turn.

### Can we make an RPA build Clairdash apps?

Yes - although admittedly, with some caveats in place. But, functionally speaking, we *can* get an RPA tool like UI.Vision to output apps in Clairdash:
...
But, if you were already using Clairdash to output these kinds of solutions, then it’s specious to imagine that shaving an extra minute or two off the build-time through RPA is going to be revolutionary - unless you’re working at an enormous scale.
